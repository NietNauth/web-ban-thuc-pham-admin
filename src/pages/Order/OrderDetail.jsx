...
const OrderDetail = () => {
  ...
  const handleUpdatePaymentStatus = async (value) => {
    try {
      setUpdating(true);
      await axiosClient.patch(`/admin/orders/${id}/payment-status`, { payment_status: value });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
         <div>
           <p className="font-medium m-0">{record.product_name || record.product?.name || 'Sản phẩm đã xóa'}</p>
         </div>
      )
    },
    {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <Space>
          ...
          <h2 className="text-xl font-bold m-0 text-gray-800">Chi tiết đơn hàng #{order.order_code || order.id}</h2>
        </Space>
        <Space>
          <Select 
            value={order.status} 
          ...
          <Button type="primary" icon={<PrinterOutlined />} onClick={handlePrintInvoice} className="bg-blue-500 hover:bg-blue-600 border-none">
            In hoá đơn
          </Button>
        </Space>
      </div>
      ...
      <Row gutter={24}>
        <Col xs={24} md={16}>
          <Card title="Danh sách sản phẩm" className="shadow-sm rounded-lg mb-6">
            <Table
              columns={columns}
                ...
                {order.discount_amount > 0 && (
                  <div className="flex justify-between mb-2">
                    <span className="text-gray-500">
                      Giảm giá {order.coupon?.code && <Tag color="blue" className="ml-1">{order.coupon.code}</Tag>}:
                    </span>
                    <span className="text-red-500 font-medium">-{formatCurrency(order.discount_amount)}</span>
                  </div>
                )}
                <Divider className="my-2" />
                <div className="flex justify-between">
                  <span className="text-lg font-bold">Tổng cộng:</span>
                  <span className="text-xl font-bold text-green-600">{formatCurrency(order.final_price)}</span>
                </div>
              </div>
            </div>
          </Card>
        ...
        <Col xs={24} md={8}>
          <Card title="Thông tin người nhận" className="shadow-sm rounded-lg mb-6 bg-green-50 bg-opacity-30">
            <div className="mb-4">
              <p className="text-gray-500 text-sm mb-1">Tên người nhận</p>
              <p className="font-medium">{order.receiver_name || order.user?.name}</p>
            </div>
            <div className="mb-4">
              <p className="text-gray-500 text-sm mb-1">Số điện thoại</p>
              <p className="font-medium">{order.receiver_phone || order.user?.phone}</p>
            </div>
            <div className="mb-4">
            ...
            <div className="mb-4">
              ...
              <p className="font-medium text-red-500">{order.note || 'Không có'}</p>
            </div>
          </Card>
          ...
          <Card title="Thông tin thanh toán" className="shadow-sm rounded-lg">
            ...
            <div className="mb-4">
              ...
              <Select 
                ...
                loading={updating}
                style={{ width: '100%' }}
                className={order.payment_status === 'paid' ? 'status-paid' : 'status-unpaid'}
              >
